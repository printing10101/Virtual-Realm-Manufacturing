...
router_content = '''<template>
  <div class="content-card">
    ...
    <div class="content-card__body">
      ...
      >
        <el-empty
          :description="t('routerStatusPanel.emptyDescription')"
          :image-size="60"
        />
      </div>
      ...
      <template v-else>
        ...
        <div class="status-grid">
          <div class="status-item">
            <span class="status-item__label">{{ t('routerStatusPanel.currentStrategy') }}</span>
          ...
          <div class="status-item">
            <span class="status-item__label">{{ t('routerStatusPanel.activeProvider') }}</span>
            <span class="status-item__value">
              <span
              ...
              <span
                v-else
                class="empty-text"
              >{{ t('routerStatusPanel.notActivated') }}</span>
            </span>
          </div>
          <div class="status-item">
            <span class="status-item__label">{{ t('routerStatusPanel.availableProviders') }}</span>
            <span class="status-item__value">{{ store.routerStatus.available_providers }}</span>
          </div>
          <div class="status-item">
            <span class="status-item__label">{{ t('routerStatusPanel.totalLatencySamples') }}</span>
            <span class="status-item__value">{{ store.routerStatus.total_latency_samples }}</span>
          </div>
          <div class="status-item status-item--full">
            <span class="status-item__label">{{ t('routerStatusPanel.cacheHitRate') }}</span>
            <span class="status-item__value">
              <el-progress
          ...
          <div class="status-item status-item--full">
            <span class="status-item__label">{{ t('routerStatusPanel.fallbackChain') }}</span>
            <span class="status-item__value">
              ...
              <span
                v-else
                class="empty-text"
              >{{ t('routerStatusPanel.noFallbackChain') }}</span>
            </span>
          </div>
        </div>
        ...
        <div class="strategy-switcher">
          <span class="strategy-switcher__label">{{ t('routerStatusPanel.switchStrategy') }}</span>
          <el-select
            v-model="selectedStrategy"
            size="small"
            :placeholder="t('routerStatusPanel.placeholderSelectStrategy')"
            style="width: 240px;"
          ...
          <span class="strategy-switcher__hint">{{ t('routerStatusPanel.switchHint') }}</span>
        </div>
      </template>
    </div>
  </div>
...
const strategyMeta = computed(() => {
  ...
  if (!s) return { label: '-', description: t('routerStatusPanel.notLoadedYet') }
  const meta = ROUTING_STRATEGY_META.find((m) => m.value === s)
  return meta ?? { label: s, description: t('routerStatusPanel.unknownStrategy') }
})
...
with open('src/components/settings/RouterStatusPanel.vue', 'w', encoding='utf-8') as f:
    f.write(router_content)
print('RouterStatusPanel.vue updated')

# AutoDetectPanel.vue
auto_content = '''<template>
  <div class="content-card">
    <div class="content-card__header">
      <span class="content-card__title">
        <el-icon style="margin-right: 6px;"><Radar /></el-icon>
        {{ t('autoDetectPanel.title') }}
      </span>
      <div class="header-actions">
        <el-button
          size="small"
        ...
        >
          ...
          {{ t('autoDetectPanel.btnScanLocal') }}
        </el-button>
        <el-button
        ...
        >
          ...
          {{ t('autoDetectPanel.btnImportToRegistry') }}
        </el-button>
      </div>
    </div>
    <div class="content-card__body">
      <el-alert
        ...
        :title="t('autoDetectPanel.emptyTitle')"
        type="info"
        :closable="false"
        show-icon
      >
        <div>
          {{ t('autoDetectPanel.emptyDescription') }}
        </div>
      </el-alert>
      ...
      <div
        ...
        :element-loading-text="t('autoDetectPanel.loadingText')"
      />
      ...
      <template v-if="store.detected.length > 0 && !store.detecting">
        <div class="detect-summary">
          <el-tag
            ...
            size="small"
          >
            {{ t('autoDetectPanel.scanSummary', { total: store.detected.length, detected: detectedCount }) }}
          </el-tag>
          <span
            v-if="store.lastDetectDuration"
            class="detect-duration"
          >
            {{ t('autoDetectPanel.durationText', { duration: store.lastDetectDuration }) }}
          </span>
        </div>
        ...
        >
          <el-table-column
            :label="t('autoDetectPanel.colStatus')"
            width="80"
          >
            <template #default="{ row }">
              <el-tag
                ...
                effect="dark"
              >
                {{ t('autoDetectPanel.statusOnline') }}
              </el-tag>
              <el-tag
                v-else
                type="info"
                ...
                effect="plain"
              >
                {{ t('autoDetectPanel.statusOffline') }}
              </el-tag>
            </template>
          </el-table-column>

          <el-table-column
            :label="t('autoDetectPanel.colType')"
            width="140"
          >
            <template #default="{ row }">
              <el-tag
          ...
          <el-table-column
            :label="t('autoDetectPanel.colSuggestedId')"
            min-width="160"
          >
            <template #default="{ row }">
          ...
          <el-table-column
            :label="t('autoDetectPanel.colDefaultModel')"
            min-width="140"
          >
            <template #default="{ row }">
          ...
          <el-table-column
            :label="t('autoDetectPanel.colDetectionMethod')"
            width="140"
          >
            <template #default="{ row }">
          ...
          <el-table-column
            :label="t('autoDetectPanel.colDescription')"
            min-width="200"
          >
            <template #default="{ row }">
...
async function handleImport(): Promise<void> {
  try {
    await ElMessageBox.confirm(
      t('autoDetectPanel.confirmImportMessage'),
      t('autoDetectPanel.confirmImportTitle'),
      {
        confirmButtonText: t('autoDetectPanel.btnConfirmImport'),
        cancelButtonText: t('autoDetectPanel.btnCancel'),
        type: 'info',
      },
    )
...
with open('src/components/settings/AutoDetectPanel.vue', 'w', encoding='utf-8') as f:
    f.write(auto_content)
print('AutoDetectPanel.vue updated')
