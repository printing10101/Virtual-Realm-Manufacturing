...
def parse_args():
    parser = argparse.ArgumentParser(
        description="DL-LNN Phase 1: 可学习延迟 + 课程式训练"
    )
    parser.add_argument("--epochs", nargs=3, type=int, default=[100, 150, 50],
                        help="三阶段 epoch 数（默认: 100 150 50）")
    ...
    parser.add_argument("--batch", type=int, default=32,
                        help="批次大小（默认: 32）")
    parser.add_argument("--lr", nargs=3, type=float, default=[1e-3, 5e-4, 1e-4],
                        help="三阶段学习率（默认: 1e-3 5e-4 1e-4）")
    parser.add_argument("--tau-init", type=float, default=0.1,
                        help="延迟 τ 初始值 (s)（默认: 0.1）")
    parser.add_argument("--lambda-tau", type=float, default=0.01,
                        help="τ 正则化系数（默认: 0.01）")
    parser.add_argument("--no-physics-tau", action="store_true",
                        help="禁用 τ 物理正则化（消融）")
    parser.add_argument("--device", type=str, default="cuda",
                        help="设备（默认: cuda）")
    parser.add_argument("--seed", type=int, default=42,
                        help="随机种子（默认: 42）")
    parser.add_argument("--output", type=str, default="phase1_dlnn_v2/results",
                        help="输出目录")
    parser.add_argument("--no-horizon", action="store_true",
                        help="使用标量预测代替长时域预测（消融）")
    parser.add_argument("--multi-dataset", action="store_true",
                        help="启用多数据集验证（Synthetic+NUAA+NIST+6061-T6）")
...
def main():
    ...
    # 指标 JSON
    results = {
        "config": {k: str(v) if isinstance(v, (torch.Tensor, np.ndarray)) else v
