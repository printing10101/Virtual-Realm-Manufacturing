...
def load_points():
    ...
    for r in rows:
        pts.append({
            'source': r['source'], 'n': float(r['n_rpm']), 'ap': float(r['ap_mm']),
            'ae': float(r['ae_mm']), 'D': float(r['tool_diameter_mm']),
            'z': int(r['num_teeth']), 'H': float(r['hardness_hb']),
            'stable': int(r['stable']),
        })
    return pts
    ...
    with open(FRF_CSV, encoding='utf-8-sig') as f:
        for r in csv.DictReader(f):
            ...
            if int(r['mode']) != 1:
                continue
            frf[(oh, r['axis'])] = {
                'f': float(r['freq_hz']), 'zeta': float(r['damping_ratio']), 'm': float(r['modal_mass_kg']),
            }
    return frf


def tlusty_a_lim(model, n, H, D, z, ae):
    return float(model.compute_limiting_depth(
        np.array([n]), hardness=np.array([H]), tool_diameter=np.array([D]),
        num_teeth=np.array([z]), feed_rate=None, radial_depth=np.array([ae]),
    )[0])
...
def metrics(y_true, y_pred):
    ...
    m = {
        ...
        'n': int(len(yt)), 'n_stable': int((yt == 1).sum()), 'n_unstable': int((yt == 0).sum()),
    }
    return m
...
def main():
    ...
    def pred_C(pt):
        feats = build_physics_features_7d(
            spindle_speed=np.array([pt['n']]), feed_rate=np.array([0.25]),
            axial_depth=np.array([pt['ap']]), radial_depth=np.array([pt['ae']]),
            hardness=np.array([pt['H']]), tool_diameter=np.array([pt['D']]),
    ...
    # 逐点表
    print('\n=== 逐点对比（实测 vs 各模型预测）===')
    print(f'{"点":<36}{"n":>7}{"ap":>6}{"实测":>5}{"A默认":>6}{"A_alim":>8}{"B真实":>6}{"B_alim":>8}{"C_DLNN":>7}{"C_alim":>8}')
    for i, pt in enumerate(pts):
        a_s = results['A_default_tlusty']
        b_s = results['B_real_modal_tlusty']
        c_s = results['C_dlnn']
        print(f'{pt["source"][:35]:<36}{pt["n"]:>7.0f}{pt["ap"]:>6.1f}{pt["stable"]:>5}'
    ...
    out = {
        ...
        'n_points': len(pts),
        'points': [{'source': p['source'], 'n_rpm': p['n'], 'ap_mm': p['ap'], 'measured_stable': p['stable']} for p in pts],
        'models': results,
        'notes': [
