...
# 失稳区域阴影：切深高于叶瓣包络 → 颤振
ax.fill_between(n / 1000, a_lim, a_lim.max() * 1.25, color="tab:red", alpha=0.12,
                label="chatter (unstable) region")
# 稳定区域（包络下方）
ax.fill_between(n / 1000, 0, a_lim, color="tab:green", alpha=0.08,
                label="stable region")

ax.axhline(0, color="k", linewidth=0.8)
ax.set_xlabel("Spindle speed n (krpm)")
ax.set_ylabel(r"Limiting axial depth of cut $a_{lim}$ (mm)")
ax.set_title("Tlusty stability lobe diagram (SDOF, k=1 MN/m, m=100 kg, "
...
ax.annotate("peak lobe (sweet spot)", xy=(n[i_max] / 1000, a_lim[i_max]),
...
print(f"a_lim range: {a_lim.min():.3f} .. {a_lim.max():.3f} mm | "
      f"peak at n={n[i_max]:.0f} rpm ({a_lim[i_max]:.3f} mm)")
