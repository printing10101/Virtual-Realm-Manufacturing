...
def fig5_mc() -> str:
    ...
    ax.text(q05, ax.get_ylim()[1] * 0.92, f"P5={q05:.2f}×", ha="center",
            fontsize=9, color="#C44E52")
    ax.text(q50, ax.get_ylim()[1] * 0.82, f"中位={q50:.2f}×", ha="center",
            fontsize=9, color="#C44E52")
    ax.text(q95, ax.get_ylim()[1] * 0.92, f"P95={q95:.2f}×", ha="center",
            fontsize=9, color="#C44E52")
...
def fig6_materials() -> str:
    ...
    for m in data["materials"]:
        ...
        g_lo_r03.append(m["gain_r0.3"][0]); g_hi_r03.append(m["gain_r0.3"][1])
        g_lo_r10.append(m["gain_r1.0"][0]); g_hi_r10.append(m["gain_r1.0"][1])
        ev.append("锚点" if m["evidence"].startswith("✅") else
                  ("推导" if m["evidence"].startswith("⚠️") else "区间"))
    x = np.arange(len(mats)); w = 0.36
    fig, ax = plt.subplots(figsize=(6.8, 3.8))
    for lo, hi, off, lab, c in [(g_lo_r03, g_hi_r03, -w / 2, "r=0.3（集中加热）", "#4C72B0"),
                                (g_lo_r10, g_hi_r10, w / 2, "r=1.0（等温极限）", "#DD8452")]:
        ax.bar(x + off, np.array(hi) - np.array(lo), w, bottom=lo,
    ...
    fig.tight_layout(); fig.savefig(OUT / "fig6_materials.png"); plt.close(fig)
...
def fig7_lnn() -> str:
    ...
    ss_res = float(np.sum((y_te - y_hat) ** 2)); ss_tot = float(np.sum((y_te - y_te.mean()) ** 2))
    axes[0].set_xlabel("真值裕度 margin"); axes[0].set_ylabel("代理预测")
    ...
    fig.tight_layout(); fig.savefig(OUT / "fig7_lnn.png"); plt.close(fig)
