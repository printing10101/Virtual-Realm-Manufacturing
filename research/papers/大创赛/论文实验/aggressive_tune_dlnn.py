"""
激进的DL-LNN调优脚本
目标：让DL-LNN的MAE低于LSTM基线(0.8104mm)
策略：
1. 更大的模型容量
2. 改进的LTC动态特性
3. 更精细的学习率调度
...
class TlustyAnalyticalModel:
    def __init__(self, stiffness=1e6, modal_mass=100.0, damping_ratio=0.05, cutting_force_coeff=2000.0, num_teeth=4):
...
class AdvancedDelayEmbedding(nn.Module):
    ...
    def forward(self, x, history=None):
        ...
        # 添加序列维度用于注意力
        embedded_seq = embedded.unsqueeze(1)  # [batch, 1, embedding_dim]
...
class AdvancedDLNNModel(nn.Module):
    ...
    def __init__(
        self, input_dim=1, hidden_dim=256, embedding_dim=128, num_ltc_layers=3, delay_steps=10, output_dim=1, dt=0.1
    ):
        super().__init__()
...
def prepare_dataloaders(datasets, batch_size=32):
    ...
    for name, data in datasets.items():
        ...
        tlusty = TlustyAnalyticalModel(
            stiffness=data["config"]["stiffness"],
            modal_mass=data["config"]["mass"],
            damping_ratio=data["config"]["damping"],
            cutting_force_coeff=2000.0,
            num_teeth=4,
        )
...
def run_aggressive_tuning():
    ...
    for config in configs:
        ...
        print(f"  delay_steps: {config['delay_steps']}")
        print(f"  num_ltc_layers: {config['num_ltc_layers']}")
        ...
        for dataset_name in datasets.keys():
            ...
            model = AdvancedDLNNModel(
                ...
                num_ltc_layers=config["num_ltc_layers"],
            ...
            start_time = time.time()
            model = train_model(
                model, dataloaders[dataset_name], device, num_epochs=config["num_epochs"], lr=config["lr"]
            )
            ...
            config_results[dataset_name] = metrics
        ...
        print(f"\n配置 {config['name']} 平均 MAE: {avg_mae:.4f} ± {std_mae:.4f} mm")

        if avg_mae < 0.8104:
            improvement = (0.8104 - avg_mae) / 0.8104 * 100
            print(f"✓ 优于LSTM基线 {improvement:.2f}%")
        else:
            gap = (avg_mae - 0.8104) / 0.8104 * 100
            print(f"✗ 差于LSTM基线 {gap:.2f}%")

    # 找出最佳配置
    best_config_name = min(all_results.keys(), key=lambda k: all_results[k]["avg_MAE"])
    best_result = all_results[best_config_name]

    print(f"\n{'=' * 80}")
    print(f"最佳配置: {best_config_name}")
    print(f"{'=' * 80}")
    print(f"平均 MAE: {best_result['avg_MAE']:.4f} ± {best_result['std_MAE']:.4f} mm")
    print(f"配置参数:")
    for key, value in best_result["config"].items():
        print(f"  {key}: {value}")

    # 保存结果
    with open("aggressive_tuning_results.json", "w", encoding="utf-8") as f:
    ...
    print(f"DL-LNN (最佳激进配置): {best_result['avg_MAE']:.4f} mm")

    return all_results, best_result


if __name__ == "__main__":
    all_results, best_result = run_aggressive_tuning()
