...
class MillingStabilityDataset(Dataset):
    ...
    def __init__(self, data_dict: Dict, dataset_name: str):
        ...
        print(f"✓ {dataset_name}: {len(self.X)} samples, "
...
class AdvancedDLNNModel(nn.Module):
    """高级DL-LNN模型"""
    def __init__(self, input_dim=6, hidden_dim=256, embedding_dim=128,
...
def main():
    ...
    for dataset_name, data_dict in all_data.items():
        ...
        dlnn_model = AdvancedDLNNModel(
            input_dim=6, hidden_dim=256, embedding_dim=128,
            num_ltc_layers=3, delay_steps=10, dt=0.1
        )
