...
pub async fn export_logs_cmd<R: Runtime>(
    ...
    fn walk_logs(dir: &std::path::Path, cutoff_ts: i64, out: &mut Vec<String>, count: &mut u64, size: &mut u64) {
        if let Ok(entries) = std::fs::read_dir(dir) {
            for entry in entries.flatten() {
                let path = entry.path();
                if path.is_dir() {
                    walk_logs(&path, cutoff_ts, out, count, size);
    ...
    walk_logs(&log_dir, cutoff_ts, &mut matched, &mut file_count, &mut total_size);
