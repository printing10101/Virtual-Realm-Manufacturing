    ...
    try:
        ...
        if "from app.core.exceptions import" not in content:
            ...
            content = content.replace(
                ...
                content.split("from app.ai.llm.provider_base import(")[1].split(")")[0].replace("from app.ai.llm.provider_base import(", "")
            )
        ...
        # 修复为带详细记录的捕获
        pattern_leading_code = r"except\s+Exception\s+as\s+e:\s*#\s*noqa:\s*BLE001\s*\n\s*logger\.debug\(.*?\n\s*return.*?\n"
...
'''
        ...
        # 先处理更具体的异常类型
        content = re.sub(
            r"except\s+\(ConnectionError.*?\n",
            replace_provider_exception,
            content,
            count=1
        )
        ...
        if "async def detect" in content or "async def health_check" in content:
            ...
            content = re.sub(
                ...
                lambda m: f"{m.group(1)}        return await self._circuit_breaker.execute(self._execute_detect)" +
                          f"\n{m.group(2)}",
                content,
                flags=re.DOTALL
            )
