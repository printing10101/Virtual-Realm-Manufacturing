...
def draw_icon(size: int) -> Image.Image:
    ...
    # 浅色齿轮（略小，偏移一点点模拟高光）
    draw_gear(gear_draw, cx - 0.02 * S, cy - 0.02 * S,
              gear_outer_r * 0.92, gear_inner_r * 0.92, gear_hub_r, n_teeth, BLUE_LIGHT + (180,))
    # 重新画主齿轮（覆盖回去，让浅色只作为边缘高光）
    draw_gear(gear_draw, cx, cy, gear_outer_r * 0.95, gear_inner_r * 0.95, gear_hub_r, n_teeth, BLUE_BRIGHT + (255,))

    # 齿轮中心圆孔（深色，准备放立方体）
    gear_draw.ellipse([cx - gear_hub_r, cy - gear_hub_r, cx + gear_hub_r, cy + gear_hub_r],
                      fill=BG_DEEP + (255,))
    ...
    cube_draw.line([top_top, top_right, top_bottom, top_left, top_top],
                   fill=edge_color, width=max(1, int(0.5 * SCALE)))
    ...
    # 外圈深色描边
    border_draw.ellipse([border_w, border_w, S - 1 - border_w, S - 1 - border_w],
                        outline=(0, 0, 0, 200), width=border_w)
    # 内圈蓝色高光
    border_w2 = max(1, int(S * 0.006))
    border_draw.ellipse([border_w2 * 2, border_w2 * 2, S - 1 - border_w2 * 2, S - 1 - border_w2 * 2],
                        outline=BLUE_BRIGHT + (100, ), width=border_w2)
