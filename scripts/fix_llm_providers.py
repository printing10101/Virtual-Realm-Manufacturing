...
"""
            ...
            (
                r"(\n.*?except\s+Exception\s+as\s+e:\s*#\s*noqa:\s*BLE001\s*\n\s*logger\.info\(.*?\))",
                lambda m: (
                    m.group(1).replace("logger.info", "logger.exception")
                    + "\n            raise LLMException(provider=self.config.provider_type.name, message=str(e)) from e"
                ),
            ),
        ]
