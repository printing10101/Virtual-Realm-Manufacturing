...
**回归告警阈值**: ±10.0%
**数值对比**: 40 matched, 0 regressions, 19 improvements, 4 missing
...
| `TestAuditLogHashChainPerformance::test_hash_chain_overhead` | overhead_vs_plain | 50.1% | -2.2% | -104.4% |
...
**统计**: ✅ 40 matched, ⚠️ 0 regressions, 🎉 19 improvements, ❓ 4 missing
...
| 🎉 | `TestAuditLogHashChainPerformance::test_hash_chain_overhead` | overhead_vs_plain | 50.1% | -2.2% | -104.4% | ↓ 越小越好 | SHA-256 on short payloads is very fast. 输出格式: 'Overhead: X%' |
...
1. **性能提升确认**：19 项指标有改善，若属优化成果建议更新 BASELINE.json 锚点值以固化为新基线。
