...
class TestDataFlywheelManifest:
    ...
    def test_manifest_required_capabilities_in_builtin(self):
        ...
        for cap in m.required_capabilities:
            assert cap in BUILTIN_CAPABILITIES, (
                f"required_capability '{cap}' 不在内置能力清单"
            )
    ...
    def test_manifest_optional_capabilities_in_builtin(self):
        ...
        for cap in m.optional_capabilities:
            assert cap in BUILTIN_CAPABILITIES, (
                f"optional_capability '{cap}' 不在内置能力清单"
            )
    ...
    def test_manifest_config_schema_top_level_sections(self):
        ...
        for section in expected_sections:
            ...
            assert section in required, (
                f"section '{section}' 未列入 required"
            )
...
class TestDataFlywheelPluginLifecycle:
    ...
    def test_on_load_registers_three_extension_points(
        ...
        panel_contribs = fresh_registry.list(
        ...
        tmpl_contribs = fresh_registry.list(
    ...
    def test_on_load_workflow_template_metadata(
        self, plugin_instance, fresh_registry: ExtensionRegistry
    ):
    ...
    def test_health_check_after_load_healthy(
        self, plugin_instance, fresh_registry: ExtensionRegistry
    ):
    ...
    def test_health_check_partial_context(
        self, plugin_instance, fresh_registry: ExtensionRegistry
    ):
    ...
    def test_on_unload_clears_extensions(
        self, plugin_instance, fresh_registry: ExtensionRegistry
    ):
        ...
        assert (
            fresh_registry.count(BUILTIN_EXTENSION_POINTS.UI_WORKSPACE_PANEL) == 0
        )
        assert (
            fresh_registry.count(BUILTIN_EXTENSION_POINTS.WORKFLOW_TEMPLATE) == 0
        )
        assert (
            fresh_registry.count(BUILTIN_EXTENSION_POINTS.TASK_HANDLER) == 0
        )
...
class TestDataFlywheelWorkflowTemplateHandler:
    ...
    def test_invoke_workflow_template_returns_ready(
        self, plugin_instance, fresh_registry: ExtensionRegistry
    ):
    ...
    def test_invoke_workflow_template_via_invoke_first(
        self, plugin_instance, fresh_registry: ExtensionRegistry
    ):
    ...
    def test_invoke_first_without_registration_returns_default(
        self, fresh_registry: ExtensionRegistry
    ):
...
class TestDataFlywheelReloadCycle:
    ...
    def test_reload_after_unload_registers_again(
        self, plugin_instance, fresh_registry: ExtensionRegistry
    ):
