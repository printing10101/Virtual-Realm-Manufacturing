...
导致 LNNModelRegistry 的 HybridLNN 映射为 None。2026-09 自
research/models/hybrid_lnn.py 回迁；训练转换路径（to_torch/_train_step
的 torch 分支）依赖 research 侧的 torch 变体，运行时仅使用 NumPy 推理。
"""
...
class HybridLNNModel(BaseLNNModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 融合层参数
        self.fusion_weights: np.ndarray | None = None
        self.fusion_bias: np.ndarray | None = None
    ...
    def predict_multimodal(self, structured_data: np.ndarray, image_data: np.ndarray | None = None) -> np.ndarray:
        """
        多模态预测
    ...
    def _train_step(
        ...
        Note:
            训练转换依赖 research 侧的 ``models.torch_hybrid_lnn``（torch 变体），
            工程运行时不含该依赖——训练请移步 research 环境。
    ...
    def to_torch(self, device: str = "cpu"):
        ...
        Note:
            依赖 research 侧的 ``models.torch_hybrid_lnn``（torch 变体），
            工程运行时不含该依赖——调用将抛出带指引的 ModuleNotFoundError。
