...
导致 LNNModelRegistry 的 LTC 映射为 None。2026-09 自
research/models/ltc_model.py 回迁；训练转换路径（to_torch）依赖
research 侧的 torch 变体，运行时仅使用 NumPy 推理。
"""
...
class LTCModel(BaseLNNModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.memory_state: np.ndarray | None = None
    ...
    def to_torch(self, device: str = "cpu"):
        ...
        Note:
            训练转换依赖 research 侧的 ``models.torch_ltc_model``（torch 变体），
        ...
        try:
            ...
            raise RuntimeError(
                "LTC 模型转换失败：转换为 PyTorch 张量需要安装 PyTorch 库（工程运行时"
