...
class KoboldCppProvider(LLMProvider):
    ...
    def __init__(self, config: ProviderConfig) -> None:
        ...
        # 确保类型正确
        config.provider_type = ProviderType.KOBOLDCPP
    ...
    async def detect(self) -> bool:
        ...
        try:
            ...
            logger.debug("KoboldCpp detect failed: %s", e)
            return False
    ...
    async def health_check(self) -> ProviderStatus:
        ...
        try:
            ...
            logger.debug("KoboldCpp health check failed: %s", e)
            self._update_status(ProviderStatus.OFFLINE)
    ...
    async def list_models(self) -> list[str]:
        ...
        try:
            ...
            logger.warning("KoboldCpp list_models failed: %s", e)
            return []
    ...
    async def chat_completion(
        ...
        """调用 KoboldCpp /v1/chat/completions 端点（OpenAI 兼容）。"""
        ...
        if response.status_code != 200:
            raise ProviderError(f"KoboldCpp API error: {response.status_code} - {response.text}")
        data = response.json()
        self._update_status(ProviderStatus.ONLINE)
