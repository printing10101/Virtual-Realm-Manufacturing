...
TGI 默认端口 8090，新版本提供 OpenAI 兼容 API：
- GET  /v1/models
- POST /v1/chat/completions

TGI 启动命令通常为：text-generation-launcher --model-id <model>
                      --port 8090。
"""
...
class TGIProvider(LLMProvider):
    ...
    def __init__(self, config: ProviderConfig) -> None:
        ...
        # 确保类型正确
        config.provider_type = ProviderType.TGI
    ...
    async def detect(self) -> bool:
        ...
        try:
            ...
            logger.debug("TGI detect failed: %s", e)
            return False
    ...
    async def health_check(self) -> ProviderStatus:
        ...
        try:
            ...
            logger.debug("TGI health check failed: %s", e)
            self._update_status(ProviderStatus.OFFLINE)
    ...
    async def list_models(self) -> list[str]:
        ...
        try:
            ...
            logger.warning("TGI list_models failed: %s", e)
            return []
    ...
    async def chat_completion(
        ...
        """调用 TGI /v1/chat/completions 端点（OpenAI 兼容）。"""
        ...
        if response.status_code != 200:
            raise ProviderError(f"TGI API error: {response.status_code} - {response.text}")
        data = response.json()
        self._update_status(ProviderStatus.ONLINE)
