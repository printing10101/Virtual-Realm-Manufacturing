...
from app.config.limits import DEFAULT_QUERY_LIMIT, MAX_EXPORT_LIMIT
...
# ``DEFAULT_QUERY_LIMIT``（默认查询条数上限）与 ``MAX_EXPORT_LIMIT``（导出条数上限）
# 由 ``app.config.limits`` 集中管理，与 rules/api.py / rag/knowledge_base.py /
# audit/reader.py 共享同一基准值。
