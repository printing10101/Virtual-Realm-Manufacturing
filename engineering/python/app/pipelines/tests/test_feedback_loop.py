...
def sample_machining_record():
    ...
    return {
        ...
        "actual_dimensions": [
            {"dimension": "length", "actual": 100.05, "nominal": 100.0, "tolerance": 0.1}
        ],
...
def feedback_pipeline(feedback_updater, training_data_lake):
    ...
    return FeedbackLoopPipeline(
        ...
        training_data_lake=training_data_lake
    )
...
class TestFeedbackLoopPipeline:
    ...
    async def test_retry_mechanism(self, feedback_pipeline, sample_machining_record):
        ...
        # 模拟训练数据湖写入失败
        with patch.object(
            feedback_pipeline.training_data_lake,
