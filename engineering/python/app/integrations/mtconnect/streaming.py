...
class StreamConsumer:
    ...
    async def on_event(self, event: StreamEvent) -> None:
        ...
        if event.event_type == "alert":
            self.alert_count += 1
            logger.warning("[%s] Alert: %s (priority=%d)", 
                         self.name, event.message, event.priority)
...
class MTConnectStreamServer:
    ...
    async def stream(self) -> AsyncIterator[StreamEvent]:
        ...
        try:
            ...
            self._subscriber_count -= 1
            logger.info("[%s] Subscriber removed, active=%d", 
                       self.agent_url, self._subscriber_count)
    ...
    def _check_alerts(self, sample: Sample) -> list[AlertEvent]:
        ...
        if (sample.spindle_load is not None and 
            sample.spindle_load > 80.0):
        ...
        if (sample.feedrate is not None and 
            sample.feedrate < 0.1):  # Near-zero feed rate
            alerts.append(AlertEvent(
                event_type="alert",
