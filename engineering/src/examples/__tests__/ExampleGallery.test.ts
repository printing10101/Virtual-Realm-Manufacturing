...
describe("Example Gallery Data", () => {
  ...
  it("每个分类应该至少有一个示例", () => {
    ...
    categories.forEach((category) => {
      const count = exampleProjects.filter(
  ...
  it("高级示例应该包含复杂功能", () => {
    const advancedExamples = exampleProjects.filter(
  ...
  it("工具路径示例应该包含CNC相关内容", () => {
    const toolpathExamples = exampleProjects.filter(
      (p) => p.category === "toolpath",
    );
  ...
  it("仿真示例应该包含物理模拟相关内容", () => {
    const simulationExamples = exampleProjects.filter(
      (p) => p.category === "simulation",
    );
