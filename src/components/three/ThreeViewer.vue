<template>
  ...
  >
    ...
    <div
      v-if="!isLoaded && !hasError && !isLoadingThree"
      class="loading-overlay"
    >
      <el-icon class="loading-icon">
        <Loading />
...
async function loadThreeJS() {
  ...
  try {
    ...
    hasError.value = true
    isLoadingThree.value = false
    errorText.value = `3D引擎加载失败: ${error instanceof Error ? error.message : String(error)}`
  ...
  if (gridHelper) {
    ...
    if (gridHelper.material) {
      if (Array.isArray(gridHelper.material)) {
        gridHelper.material.forEach((mat: any) => mat.dispose())
      } else {
