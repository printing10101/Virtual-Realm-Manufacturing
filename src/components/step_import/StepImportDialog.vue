<template>
  ...
  >
    <el-tabs v-model="activeTab" @tab-change="onTabChange">
      <el-tab-pane label="导入模型" name="import">
        <div class="step-import-container">
      <!-- 导入前：文件选择区 -->
      <div v-if="!store.isSuccess && !store.isError" class="upload-section">
        <el-upload
          ref="uploadRef"
          class="step-uploader"
          drag
          ...
          <el-form label-width="80px" size="small">
            <el-form-item label="输出格式">
              <el-radio-group v-model="outputFormat">
                <el-radio value="stl">STL (三角网格)</el-radio>
                <el-radio value="brep">BREP (边界表示)</el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item label="精度级别">
              <el-select v-model="precision" style="width: 160px">
                <el-option label="低精度 (快速)" value="low" />
                <el-option label="中精度 (平衡)" value="medium" />
                <el-option label="高精度 (精细)" value="high" />
              </el-select>
      ...
      <!-- 上传/处理进度 -->
      <div v-if="store.isActive" class="progress-section">
        <div class="progress-status">
          <el-icon v-if="store.isUploading" class="is-loading"><loading /></el-icon>
        ...
        <el-progress
          ...
          :stroke-width="8"
        />
        <div class="progress-detail" v-if="store.isProcessing">
          正在解析STEP几何数据并生成三角网格，请耐心等待...
        </div>
      </div>

      <!-- 成功结果 -->
      <div v-if="store.isSuccess && store.currentResult" class="result-section">
        <el-alert
        ...
        <!-- 包围盒尺寸 -->
        <div class="model-dimensions" v-if="store.modelInfo?.bounding_box">
          <h4>包围盒尺寸</h4>
          ...
          <div class="dim-extra" v-if="store.modelInfo.volume > 0">
        ...
        <!-- 警告信息 -->
        <div v-if="store.warnings.length > 0" class="warning-section">
          <el-alert
        ...
        <!-- 多实体选择 -->
        <div v-if="store.hasStlFiles && store.activeStlFiles.length > 1" class="entity-selector">
          <h4>实体选择 ({{ store.activeStlFiles.length }} 个实体)</h4>
          <el-radio-group v-model="entityIndex" @change="onEntityChange" size="small">
            <el-radio-button
        ...
        <!-- 转换错误 -->
        <div v-if="store.currentResult.status.errors.length > 0" class="error-detail">
          <el-alert
      ...
      <!-- 错误状态 -->
      <div v-if="store.isError" class="error-section">
        <el-result icon="error" title="导入失败" :sub-title="store.errorMessage">
          <template #extra>
      ...
      <el-tab-pane label="导入历史" name="history">
        <div class="history-container">
          ...
          <el-table
            ...
            size="small"
          >
            <el-table-column prop="original_name" label="原始文件" min-width="180" show-overflow-tooltip />
            ...
            <el-table-column label="操作" width="160">
              <template #default="{ row }">
                <el-button size="small" text type="primary" @click="handlePreviewHistory(row)">
                  查看
                </el-button>
                <el-button size="small" text type="danger" @click="handleDeleteHistory(row)">
                  删除
                </el-button>
              </template>
            </el-table-column>
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button @click="handleClose">{{ store.isSuccess ? '完成' : '取消' }}</el-button>
      </div>
    </template>
  </el-dialog>
