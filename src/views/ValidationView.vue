<template>
  <div class="validation-view">
    ...
    <el-row :gutter="20">
      <el-col :span="8">
        <el-card shadow="hover" class="left-panel">
            ...
            >
              <div class="dataset-info">
                ...
                <div class="dataset-meta">
                  <el-tag size="small">{{ ds.samples }} 样本</el-tag>
                  <el-tag size="small" type="info">{{ ds.source }}</el-tag>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="import-section">
            ...
            >
              <el-button type="primary" :icon="Upload">导入自定义数据集</el-button>
            </el-upload>
            <el-input
              v-model="importName"
              placeholder="数据集名称"
        ...
        <el-card shadow="hover" class="config-panel" style="margin-top: 20px">
          <template #header>
            <div class="card-header">
              <span>
          ...
          <el-form label-position="top" size="small">
            <el-form-item label="验证类型">
              <el-select v-model="validationType" style="width: 100%">
                <el-option label="在线公式验证" value="online" />
                <el-option label="离线数据集验证" value="offline" />
                <el-option label="综合验证" value="comprehensive" />
              </el-select>
            </el-form-item>

            <el-form-item label="切削速度 (m/min)">
              <el-input-number v-model="params.v_c" :min="10" :max="1000" :step="10" style="width: 100%" />
            </el-form-item>

            <el-form-item label="进给量 (mm/rev)">
              <el-input-number v-model="params.f" :min="0.01" :max="5" :step="0.05" style="width: 100%" />
            </el-form-item>

            <el-form-item label="切削深度 (mm)">
              <el-input-number v-model="params.a_p" :min="0.1" :max="50" :step="0.5" style="width: 100%" />
            </el-form-item>

            <el-form-item label="材料">
              <el-select v-model="params.material" style="width: 100%">
                <el-option label="45钢" value="45钢" />
                <el-option label="不锈钢304" value="不锈钢304" />
                <el-option label="铝合金6061" value="铝合金6061" />
                <el-option label="钛合金TC4" value="钛合金TC4" />
              </el-select>
            </el-form-item>
            ...
            <el-form-item label="误差阈值配置">
              <el-row :gutter="10">
                <el-col :span="8">
                  <el-input-number v-model="thresholds.cutting_force" :min="1" :max="50" :step="1" size="small" style="width: 100%" />
                  <div class="threshold-label">切削力 %</div>
                </el-col>
                <el-col :span="8">
                  <el-input-number v-model="thresholds.tool_life" :min="1" :max="50" :step="1" size="small" style="width: 100%" />
                  <div class="threshold-label">刀具寿命 %</div>
                </el-col>
                <el-col :span="8">
                  <el-input-number v-model="thresholds.surface_roughness" :min="1" :max="50" :step="1" size="small" style="width: 100%" />
                  <div class="threshold-label">粗糙度 %</div>
                </el-col>
              </el-row>
            </el-form-item>
      ...
      <el-col :span="16">
        <el-card shadow="hover" class="results-panel">
          <template #header>
            <div class="card-header">
              <span>
          ...
          <el-empty v-if="!validationResult" description="运行验证后显示结果" />

          <template v-if="validationResult">
            <div class="metrics-cards">
              <el-card shadow="hover" class="metric-card" :class="metricStatus('mape')">
                <div class="metric-label">MAPE</div>
                <div class="metric-value">{{ validationResult.overall_mape?.toFixed(2) || 0 }}%</div>
                <div class="metric-desc">平均绝对百分比误差</div>
              </el-card>

              <el-card shadow="hover" class="metric-card" :class="metricStatus('rmse')">
                <div class="metric-label">RMSE</div>
                <div class="metric-value">{{ validationResult.overall_rmse?.toFixed(4) || 0 }}</div>
                <div class="metric-desc">均方根误差</div>
              </el-card>

              <el-card shadow="hover" class="metric-card" :class="metricStatus('r2')">
                <div class="metric-label">R²</div>
                <div class="metric-value">{{ validationResult.overall_r_squared?.toFixed(4) || 0 }}</div>
                <div class="metric-desc">决定系数</div>
              </el-card>

              <el-card shadow="hover" class="metric-card">
                <div class="metric-label">样本统计</div>
                <div class="metric-value">{{ validationResult.total_samples || 0 }}</div>
            ...
            >
              ...
              <el-table-column prop="predicted_value" label="预测值" width="100" />
              <el-table-column prop="actual_value" label="实际值" width="100" />
              <el-table-column prop="error" label="误差" width="100" :formatter="formatError" />
              <el-table-column prop="error_percent" label="误差%" width="100" :formatter="formatPercent" />
