...
# 创建T权限且paper_only=true的Token（用于Paper-Only测试）
t_payload = {
    "scopes": ["R", "B", "T"],
    "expires_in": 3600,
    "paper_only": True
...
assert "403" in str(error_data) or "forbidden" in str(error_data).lower() or "权限" in str(error_data) or "scope" in str(error_data).lower(), "错误响应不包含权限不足提示"
...
# 确保LNN_LIVE_EXECUTION_ENABLED=false（默认）
execute_payload = {
...
# Paper-Only模式应该记录操作但不实际执行
assert result.get("paper_only") is True or result.get("mode") == "simulation" or result.get("simulated") is True, "Paper-only模式未正确启用"
...
if resp.status_code == 200:
    ...
    if len(logs) > 0:
        ...
        # 验证必要字段
        required_fields = ["timestamp", "agent_id", "route", "permission_class", "status_code", "latency_ms"]
        ...
        assert agent_id in agent_ids, f"Token创建的agent_id未出现在审计日志中"
...
# 验证幂等性
# 第二次响应应该返回缓存的结果，且耗时更短
if result1.get("job_id") == result2.get("job_id"):
    print("[PASS] 两次请求返回相同的job_id，幂等性机制生效")
else:
    print(f"[WARN] job_id不同: {result1.get('job_id')} vs {result2.get('job_id')}")
...
revoked_count = revoke_result.get("revoked_count", 0)
