...
async function setupCommonMocks(page: Page) {
  await page.route('**/api/v1/version', (route) =>
    route.fulfill({
      ...
      body: JSON.stringify({ version: '2.3.0', commit: 'e2e-flywheel' }),
    }),
  )
  ...
  await page.route('**/api/health', (route) =>
    route.fulfill({ status: 200, body: JSON.stringify({ status: 'ok', version: '2.3.0' }) }),
  )
  await page.route('**/health', (route) =>
    route.fulfill({
      status: 200,
      body: JSON.stringify({ status: 'healthy', timestamp: Date.now(), version: '2.3.0', uptime: 7200 }),
    }),
  )
}
