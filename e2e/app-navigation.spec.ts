...
async function mockApiRoutes(page: Page) {
  await page.route('**/api/v1/version', (route) => {
    route.fulfill({
      ...
      body: JSON.stringify({ version: '1.9.0', commit: 'test-abc' }),
    })
  })
  ...
  await page.route('**/api/health', (route) => {
    route.fulfill({ status: 200, body: JSON.stringify({ status: 'ok', version: '1.9.0' }) })
  })
  await page.route('**/health', (route) => {
    route.fulfill({
      status: 200,
      body: JSON.stringify({ status: 'healthy', timestamp: Date.now(), version: '1.9.0', uptime: 3600 }),
    })
  })
