...
try:
    ...
    if code == 200:
        ...
        if job_id and elapsed < 3:
            check("Test1: Train returns job_id <3s", True, f"job_id={job_id[:12]}..., {elapsed:.2f}s")
        elif job_id:
            check("Test1: Train returns job_id <3s", False, f"Slow: {elapsed:.2f}s")
        else:
            check("Test1: Train returns job_id <3s", False, f"No job_id: {data.get('message','')}")
    else:
        check("Test1: Train returns job_id <3s", False, f"HTTP {code}: {json.dumps(data, ensure_ascii=False)[:200]}")
    ...
    if code == 200:
        ...
        check("Test7: History records", False, f"HTTP {code}: {json.dumps(data7, ensure_ascii=False)[:200]}")
