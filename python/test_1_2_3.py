...
body = json.dumps({
    ...
    "hyperparameters": {"learning_rate": 0.001, "epochs": 15, "batch_size": 32, "optimizer": "adam"},
    "device": "cpu"
}).encode()
...
print(f"TEST 1: {'PASS' if elapsed1 < 3 and result['data']['status'] == 'queued' else 'FAIL'}")
...
if event_types[-1] != "complete" and event_types[-1] != "failed":
    valid_sequence = False
    print(f"  FAIL: Last event should be 'complete' or 'failed', got '{event_types[-1]}'")
...
print("\n" + "=" * 50)
print(f"SUMMARY: Test 1 {'PASS' if elapsed1 < 3 else 'FAIL'} | Test 2 {'PASS' if pass2 else 'FAIL'} | Test 3 {'PASS' if valid_sequence and event_types[-1] == 'complete' else 'FAIL'}")
print("=" * 50)
