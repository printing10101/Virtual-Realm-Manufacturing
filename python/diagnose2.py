...
proc = subprocess.Popen(
    ...
    cwd=str(PYTHON_DIR), env=env, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True
)
...
t0 = time.time()
try:
    req = urllib.request.Request(f"{BASE}/api/v1/lnn/predict", 
        data=json.dumps({"model_name":"cutting_force","input_data":[120.5,85.3,65.1,12000.0,0.15]}).encode(),
...
t0 = time.time()
try:
    req = urllib.request.Request(f"{BASE}/api/v1/lnn/models", headers={"Authorization": f"Bearer {TOKEN}"})
    r = urllib.request.urlopen(req, timeout=10)
    print(f"  Status: {r.status}, {time.time()-t0:.2f}s")
    print(f"  Body: {r.read().decode()[:300]}")
except Exception as e:
