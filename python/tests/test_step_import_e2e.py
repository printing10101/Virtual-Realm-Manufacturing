...
async def test_api_functional():
    ...
    result("HTTP状态码 200", resp.status_code == 200,
           f"实际={resp.status_code}")

    body = resp.json()
    result("响应 code=0", body.get("code") == 0,
           f"实际={body.get('code')}")
    ...
    result(f"体积(volume) > 0, 约 {expected_volume:.2f} mm^3", volume_ok,
           f"实际={volume}" if volume else "体积字段缺失")
    result(f"体积精度验证 (偏差<5%)", volume_close,
           f"期望~{expected_volume:.2f}, 实际={volume}, 偏差={abs(volume - expected_volume) / expected_volume * 100:.2f}%" if volume_ok else "")
    ...
    if bbox_ok:
        result(f"包围盒.length ~ 100mm", abs(bbox["length"] - 100) < 2,
               f"实际={bbox['length']}")
        result(f"包围盒.width ~ 50mm", abs(bbox["width"] - 50) < 2,
               f"实际={bbox['width']}")
        result(f"包围盒.height ~ 30mm", abs(bbox["height"] - 30) < 2,
               f"实际={bbox['height']}")

    # T1.4: 检查 stl_path
    stl_files = data.get("stl_files", [])
    has_stl = len(stl_files) > 0
    result("包含STL文件列表(stl_files)", has_stl,
           f"共{len(stl_files)}个STL文件")
    ...
    if has_stl:
        ...
        result("STL文件实际存在", stl_file_exists,
               f"路径={stl_path_str}")
...
async def test_error_handling():
    ...
    result("HTTP状态码 200(业务层返回)", resp.status_code == 200,
           f"实际={resp.status_code}")

    body = resp.json()
    code_nonzero = body.get("code", 0) != 0
    result("响应code != 0(错误标识)", code_nonzero,
           f"实际code={body.get('code')}")

    message = body.get("message", "")
    has_error_msg = len(message) > 0
    result("包含错误信息(message)", has_error_msg,
    ...
    resp_corrupt = client.post(
        "/api/import/step",
        files={"file": ("corrupt.step", io.BytesIO(b"NOT_A_STEP_CONTENT_12345"), "application/octet-stream")},
...
async def test_downstream_compatibility(step_data: dict):
    ...
    if mesh_extents is not None and len(mesh_extents) >= 3:
        ...
        result(f"STL包围盒.length一致(偏差<2mm)", length_match,
               f"模型={model_bbox.get('length')}, STL={mesh_extents[0]:.1f}")
        result(f"STL包围盒.width一致(偏差<2mm)", width_match,
               f"模型={model_bbox.get('width')}, STL={mesh_extents[1]:.1f}")
        result(f"STL包围盒.height一致(偏差<2mm)", height_match,
...
"""
    ...
    if result_ok:
        ...
        result(f"移除体素数: {sim_result.removed_voxel_count}", sim_result.removed_voxel_count > 0)
        result(f"碰撞检测完成", True)
        ...
        if output_stl and output_stl.exists():
            ...
            if len(mesh.vertices) > 0 and len(out_mesh.vertices) > 0:
                ...
                if orig_bbox is not None and out_bbox is not None:
                    max_diff = max(
                        abs(orig_bbox[1][i] - out_bbox[1][i])
                        for i in range(3)
                    )
                    result(f"坐标偏差(max): {max_diff:.3f}mm < 0.1mm", max_diff < 0.1,
