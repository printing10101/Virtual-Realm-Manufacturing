...
class TestMaterialToolLibrary:
    ...
    def test_five_axis_materials(self):
        """验证五轴常用材料"""
        materials_path = Path(__file__).parents[1] / "app" / "data" / "materials.json"
        ...
        # 验证新增材料
        assert "material_h59" in material_ids  # 黄铜
        assert "material_t2" in material_ids   # 紫铜
        assert "material_abs" in material_ids  # ABS
    
    def test_five_axis_tools(self):
        """验证五轴加工刀具"""
        tools_path = Path(__file__).parents[1] / "app" / "data" / "tools.json"
        with open(tools_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        
        # data is a list, not a dict with "tools" key
        tool_series = [t["series"] for t in data]
        
        # 验证五轴刀具
        assert "ball_nose" in tool_series      # 球头铣刀
        assert "taper_ball" in tool_series     # 锥度球头刀
