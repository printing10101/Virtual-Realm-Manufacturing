...
async def test_incremental_solver():
    ...
    async for phase_result in gen:
        ...
        feedback = {"passed": True, "error_rate": 0.0, "violated_constraints": {}, "strategy": "tolerant", "warnings": []}
        solver.send_feedback(feedback)
    
    assert phase_count > 0, "No phases were executed"
    assert solver.get_state().state == SolverState.COMPLETED, f"Solver state should be COMPLETED, got {solver.get_state().state}"
...
async def test_alternating_validator_strict():
    ...
    async def failing_validation(params, requirements, metrics):
        return {
            "passed": False,
            "unmet_constraints": {"cutting_force_max": {"actual": 1200, "required": 800, "violation": 400}},
            "warnings": []
        }
    ...
    requirements = {"max_cutting_force": 800.0, "max_surface_roughness": 3.2, "min_tool_life": 30.0}
    
    result = await validator.validate(constraints, requirements)
    
    assert result.terminated_early, "Strict mode should terminate early on validation failure"
    assert not result.success, "Result should not be successful in strict mode with failing validation"
...
async def test_alternating_validator_tolerant():
    ...
    async def tolerant_validation(params, requirements, metrics):
        ...
        if call_count <= 2:
            return {
                "passed": False,
                "unmet_constraints": {"cutting_force_max": {"actual": 850, "required": 800, "violation": 50}},
                "warnings": []
            }
        return {
            "passed": True,
    ...
    if error_rate < validator.tolerance_threshold:
        print(f"  PASSED: Tolerant mode correctly continued (error {error_rate:.2%} < threshold {validator.tolerance_threshold:.2%})")
    else:
        print(f"  NOTE: Error exceeded threshold, early termination expected")
...
async def test_solver_progress_service():
    ...
    service.update_phase_progress(task_id, {
        "phase": "feasibility",
        "state": "completed",
        "parameters": {"cutting_speed": 120.0, "feed_rate": 0.15, "depth_of_cut": 2.0},
        "metrics": {"cutting_force": 750.0, "surface_roughness": 1.6, "tool_life": 60.0},
        "duration_ms": 50.0
    })
    ...
    service.complete_solving(task_id, {
        "total_phases": 4,
        "success_rate": 1.0,
        "total_time_ms": 200.0
    })
...
async def test_alternating_validator_best_effort():
    ...
    async def always_fail_validation(params, requirements, metrics):
        return {
            "passed": False,
            "unmet_constraints": {"cutting_force_max": {"actual": 1500, "required": 800, "violation": 700}},
            "warnings": []
        }
