...
def analyze_main_comparison(results):
    ...
    for dataset_name, dataset_data in data.items():
        ...
        if models:
            ...
            analysis['completeness'][dataset_name] = {
...
def analyze_cross_condition(results):
    ...
    analysis = {
        'LOMO': {
            'materials': list(data.get('LOMO', {}).keys()),
            'models': [],
            'complete': False
        },
        'LOCO': {
            'conditions': list(data.get('LOCO', {}).keys()),
            'models': [],
            'complete': False
        }
    }
...
def generate_report(paper_content, json_results, figures_status, table_status):
    ...
    if time_analysis:
        ...
        report.append(f"  - 全局统计: τ_mean={time_analysis['global_stats'].get('tau_mean', 'N/A'):.4f}, "
