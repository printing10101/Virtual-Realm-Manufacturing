...
class GCodeGenerationPipeline:
    ...
    async def run_pipeline(self, task_id: str) -> GCodeGenerationResult:
        ...
        try:
            ...
            report = self._loader.load(task.source_chatter_report_path)
            ...
            operation_plan = load_operation_plan(
                task.source_operation_plan_path
            )

            # 3. 调用 GeneratorAdapter.adapt() 生成基础 G 代码 + 特征级结果
            base_result, feature_gcode_results = self._adapter.adapt(
