...
_service_singleton: Optional["ProjectPackageService"] = None
_service_lock = threading.Lock()


def get_project_package_service() -> "ProjectPackageService":
    """获取 ProjectPackageService 单例（双重检查锁）."""
    global _service_singleton
    if _service_singleton is None:
        with _service_lock:
            if _service_singleton is None:
                _service_singleton = ProjectPackageService()
...
class ProjectPackageService:
    ...
    # ── Session 管理（与 ResourceCardService 一致） ────────────────────
