...
_service_singleton: Optional["ResourceCardService"] = None
_service_lock = threading.Lock()


def get_resource_card_service() -> "ResourceCardService":
    """获取 ResourceCardService 单例（双重检查锁）."""
    global _service_singleton
    if _service_singleton is None:
        with _service_lock:
            if _service_singleton is None:
                _service_singleton = ResourceCardService()
...
class ResourceCardService:
    ...
    # ── Session 管理（与 ProjectSyncService 一致） ────────────────────
