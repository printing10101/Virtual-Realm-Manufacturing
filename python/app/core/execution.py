...
class StructuredLogger:
    ...
    def log_execution_end(self, task_id: str, status: ExecutionStatus,
                         duration_ms: float, result_summary: Optional[str] = None) -> None:
        """记录执行结束"""
        entry = {
            "timestamp": datetime.now().isoformat(),
            "event": "execution_end",
        ...
        logger.info(
            ...
            task_id, status.value, duration_ms
        )
...
class SessionManager:
    ...
    def update_session(self, session_id: str, status: ExecutionStatus,
                      checkpoint_data: Optional[Dict[str, Any]] = None) -> None:
        """更新会话状态"""
        updates = {
            "status": status.value,
        ...
        self._conn.execute(
            f"UPDATE execution_sessions SET {set_clause} WHERE session_id = ?",
            values
        )
        self._conn.commit()
...
class TaskExecutor:
    ...
    async def execute(self, task_type: str, workspace_context: Any,
...
class ExecutionEngine:
    ...
    def __init__(self):
        from app.core.heartbeat import get_scheduler, WakeupQueue
        from app.core.budget import get_budget_manager, BudgetManager
        from app.core.workspace import get_resolver, WorkspaceResolver
        from app.core.skill_loader import get_skill_loader, SkillLoader
    ...
    async def execute_task(self, task: Any) -> ExecutionResult:
        ...
        Returns:
            执行结果
        """
        from app.core.heartbeat import ScheduledTask, get_scheduler, ScheduleStatus
        ...
        try:
            ...
            if result.status == ExecutionStatus.COMPLETED:
                scheduler.wakeup_queue.update_task_status(task_id, ScheduleStatus.COMPLETED,
                                                        last_run=time.time())
                scheduler.wakeup_queue.log_execution(
                    task_id, "completed", duration_ms=result.duration_ms,
            ...
            scheduler.wakeup_queue.update_task_status(task_id, ScheduleStatus.FAILED)
            scheduler.wakeup_queue.log_execution(task_id, "failed", error_message=str(e))
    ...
    async def _handle_failure(self, task: Any, session: ExecutionSession,
        ...
        if task.retry_count < task.max_retries:
            ...
            logger.info(
                ...
                task.task_id, task.retry_count + 1, task.max_retries, retry_delay
            )
            ...
            self.session_manager.update_session(
                ...
                checkpoint_data={"retry_count": task.retry_count, "next_run": task.next_run}
            )
        else:
            logger.error(
    ...
    def _record_resource_usage(self, task_id: str, agent_id: str, 
                              result: ExecutionResult) -> None:
        """记录资源使用"""
        ...
        get_cost_tracker().record_memory_usage(
            task_id,
            result.resource_usage.get("memory_mb", 0),
            agent_id
        )
        ...
        for session in orphaned_sessions:
            if session.retry_count < session.max_retries:
                ...
                if task:
                    ...
                    task.next_run = time.time() + self._retry_delays[session.retry_count]
                    task.status = ScheduleStatus.PENDING
    ...
    async def start_recovery_loop(self, check_interval: float = 60.0,
                                  max_interval: float = 600.0) -> None:
        """启动孤立任务检测循环（含指数退避）
