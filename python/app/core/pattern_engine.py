    ...
    def analyze_patterns(self, min_samples: int = None) -> List[Pattern]:
        ...
        with self._lock:
            ...
            for p in new_patterns:
                if not any(existing.pattern_id == p.pattern_id for existing in self._patterns):
        ...
        for key, records in groups.items():
            ...
            if success_rate >= 0.9 and avg_time > 0:
                ...
                pattern = Pattern(
                    ...
                    suggestion=f"Consider using this workflow pattern for similar tasks",
                )
    ...
    def _detect_anti_patterns(self, min_samples: int) -> List[Pattern]:
        ...
        for key, records in groups.items():
            ...
            if error_rate > 0.3:
                ...
                baseline_cost = sum(
                    r.metrics.get("resource_cost", 0)
                    for r in self._executions
            ...
            if is_anti:
                ...
                pattern = Pattern(
                    ...
                    sample_size=len(records),
                    suggestion=f"Avoid this element combination. Consider adding constraint checking.",
                )
                patterns.append(pattern)
...
def init_pattern_engine(
    ...
    with _engine_lock:
        if _engine is not None:
            _engine.close()
        _engine = PatternEngine(db_path=db_path)
        _engine.initialize()
    return _engine
