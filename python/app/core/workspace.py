...
from app.ai.lnn.core import ModelType, EngineType
...
class ModelSelector:
    ...
    @classmethod
    def find_best_model(cls, model_type: ModelType, base_dir: str, 
...
class DatasetMatcher:
    ...
    @classmethod
    def match_dataset(cls, machine_id: str, material_type: Optional[str] = None,
                     dataset_type: str = "training", base_dir: str = "data") -> Optional[str]:
        """
        匹配数据集
...
class EnvironmentInjector:
    ...
    @classmethod
    def inject_environment(cls, workspace_dir: str, machine_config: Dict[str, Any],
                          task_context: WorkspaceContext) -> Dict[str, str]:
        """
        注入环境变量
        ...
        logger.info("Injected %d environment variables for task %s", len(env), task_context.task_id)
        return env
...
class WorkspaceResolver:
    ...
    def resolve(self, task_id: str, task_type: str, 
               metadata: Optional[Dict[str, Any]] = None) -> WorkspaceContext:
        """
        解析任务工作空间
        ...
        if task_type in ("lnn_train", "lnn_predict", "wear_analysis", "vibration_analysis"):
...
_resolver: Optional[WorkspaceResolver] = None
...
def init_resolver(base_dir: Optional[str] = None) -> WorkspaceResolver:
    """初始化全局工作空间解析器"""
    global _resolver
    _resolver = WorkspaceResolver(base_dir)
    return _resolver
