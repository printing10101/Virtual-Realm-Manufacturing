...
# 单个 mesh 文件上限：200MB（PLY/STL/GLB 通常 10-100MB）
MAX_MESH_SIZE = 200 * 1024 * 1024
ALLOWED_MESH_EXTENSIONS = {".ply", ".stl", ".glb", ".gltf", ".obj"}

# pipeline 单例（懒加载，避免模块导入期就触发可选依赖加载）
...
async def run_task(task_id: str) -> dict[str, Any]:
    ...
    pipeline = _get_pipeline()
    asyncio.create_task(pipeline.run_task(task_id))
