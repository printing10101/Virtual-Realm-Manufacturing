...
def _resolve_upstream_operation_plan(
    ...
    try:
        pg_task = get_pg_store().get_task(source_parametric_geometry_task_id)
        if pg_task is None:
            logger.warning(
...
async def download_gcode(task_id: str) -> FileResponse:
    ...
    if not task.gcode_file_path:
        return JSONResponse(
            ...
            content=error(
                code=ErrorCode.NOT_FOUND,
                message="任务 G 代码文件路径为空",
            ),
        )

    output_path = Path(task.gcode_file_path)
...
async def download_report(task_id: str) -> FileResponse:
    ...
    output_path = Path(task.gcode_report_path)
    if not output_path.exists():
        return JSONResponse(
            status_code=404,
            content=error(
                code=ErrorCode.FILE_NOT_FOUND,
                message="报告 JSON 文件不存在",
            ),
        )
