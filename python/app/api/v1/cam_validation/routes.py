...
async def download_cam_report(task_id: str) -> FileResponse:
    ...
    if not task.cam_report_path:
        return JSONResponse(
            status_code=404,
            content=error(
                code=ErrorCode.NOT_FOUND,
                message="任务 CAM 校验报告路径为空",
            ),
        )

    output_path = Path(task.cam_report_path)
    if not output_path.exists():
        return JSONResponse(
            status_code=404,
            content=error(
                code=ErrorCode.FILE_NOT_FOUND,
                message="CAM 校验报告文件不存在",
            ),
        )
...
async def download_internal_report(task_id: str) -> FileResponse:
    ...
    if not task.internal_report_path:
        return JSONResponse(
            status_code=404,
            content=error(
                code=ErrorCode.NOT_FOUND,
                message="任务内部预校验报告路径为空",
            ),
        )

    output_path = Path(task.internal_report_path)
    if not output_path.exists():
        return JSONResponse(
            status_code=404,
            content=error(
                code=ErrorCode.FILE_NOT_FOUND,
                message="内部预校验报告文件不存在",
            ),
        )
