...
async def agent_predict(request: AgentPredictRequest):
    ...
    try:
        ...
        if not entry:
            return error(code=ErrorCode.NOT_FOUND, message=f"Model '{request.model_name}' not found")

        if not request.input_data or any(not isinstance(x, (int, float)) for x in request.input_data):
...
async def agent_train(request: AgentTrainRequest):
    ...
    try:
        ...
        asyncio.create_task(_run_agent_training(
            task_id, request.model_name, request.data_path, hyperparams, request.device,
        ))
...
async def create_agent_token(req: AgentTokenCreateRequest):
    ...
    try:
        ...
        return success(data=AgentTokenResponse(
        ...
        ).model_dump(), message="Agent token created successfully. Save the token now, it will not be shown again.")

    except Exception as e:
        return error(code=ErrorCode.INTERNAL_ERROR, message=f"Token creation failed: {e!s}")
