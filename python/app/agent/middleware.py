...
class AgentAuthMiddleware(BaseHTTPMiddleware):
    """Middleware for Agent API authentication, rate limiting, and audit logging."""
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Token creation endpoint is exempt (bootstrap)
        if path == "/api/agent/v1/tokens" and request.method == "POST":
            return await call_next(request)

        # Only apply to /api/agent/v1/ endpoints
        ...
        # Validate token
        agent_token = agent_token_store.validate_token(raw_token)
        ...
        required_level = await get_permission_class(request.method, path)

        # Check scope permission
        if not check_scope(scopes, required_level):
            elapsed = (time.perf_counter() - start) * 1000
            agent_audit_log.log(
                agent_id=agent_id,
                route=path,
        ...
        # Store idempotency result for successful write operations
        if required_level.value in WRITE_SCOPES and response.status_code < 400:
            idem_key = request.headers.get("Idempotency-Key")
            if idem_key:
                import json
