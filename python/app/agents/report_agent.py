...
class ReportAgent:
    ...
    async def generate_report(self, task_id: str, process_task_id: str | None = None) -> str:
                ...
                if tool:
                    with self.logger.log_step(
                        ...
                        try:
                            ...
                            error_msg = f"工具 {action_name} 执行失败: {e!s}"
                            self.reasoning_steps.append(ReACTStep(
                                step_number=step_num, step_type="action", content=error_msg,
                                tool_name=action_name, tool_input=action_params,
                                tool_output={"error": str(e)}, duration_ms=0
                            ))
                            with self.logger.log_step(
                                task_id, "report_agent", StepType.REACT_OBSERVATION,
                                input_data={"tool": action_name, "error": error_msg}
                            ) as obs_log:
                                obs_log.output = {"error": str(e)}
                else:
                    error_msg = f"工具 {action_name} 不存在，跳过此步骤"
                    self.reasoning_steps.append(ReACTStep(
                        step_number=step_num, step_type="action", content=error_msg,
                        tool_name=action_name, tool_input=action_params
                    ))
