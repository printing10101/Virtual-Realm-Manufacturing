...
class WorkOrderRequest(BaseModel):
    ...
    planned_start: Optional[datetime] = Field(
        None, description="计划开始时间", example="2024-01-01T08:00:00"
    )
    planned_end: Optional[datetime] = Field(
        None, description="计划结束时间", example="2024-01-05T17:00:00"
    )
    customer_order_no: Optional[str] = Field(
        None, description="客户订单号", example="CUST-ORD-123"
    )
...
class QualityReportRequest(BaseModel):
    ...
    inspection_type: str = Field(
        ..., description="检验类型", example="in_process"
    )
    ...
    inspection_time: datetime = Field(
        ..., description="检验时间", example="2024-01-01T10:00:00"
    )
    ...
    defect_code: Optional[str] = Field(
        None, description="缺陷代码", example="DEF-001"
    )
