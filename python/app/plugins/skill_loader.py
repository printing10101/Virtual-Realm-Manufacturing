    ...
    def _watch_loop(self) -> None:
        while not self._stop_event.is_set():
            try:
                ...
                # 文件监听扫描异常（OSError/权限错误等）不应中断守护线程
                logger.warning(
...
class SkillLoader:
        ...
        for lang, code in parsed.get("code_blocks", []):
            if lang == "python":
                try:
                    ...
                    # _compile_code 在 RestrictedPython 沙箱中执行用户代码，
                    # 可抛出：SyntaxError/NameError/AttributeError/TypeError/
                    # RuntimeError/OSError（受限 IO）等。
                    # 单一代码块加载失败不应中断整个技能文件加载。
                    logger.warning(
    ...
    # =========================================================================
    # 安全沙箱设计原则 (Security Sandbox - Defense in Depth)
    # =========================================================================
    # 技能代码来自不受信任的 Markdown 文件，必须在一个受限的执行环境中运行。
    # 我们的实现采用纵深防御（Defense in Depth）策略，叠加多层防护：
    #
    # 第一层：严格白名单内置函数（_SAFE_BUILTINS）——本类
    #   - 仅包含 21 个经过逐项安全审计的纯计算函数
    #   - 字典以 "__builtins__" 名注入到执行命名空间，替换 Python 默认内置
    #   - 任何在白名单之外的名称（包括 __import__/open/exec/eval/getattr 等）
    #     在沙箱内均不可见，从而在语言层面阻止绝大多数逃逸尝试
    #   - 危险函数清单同时记录在 _FORBIDDEN_BUILTINS 中作为审计与文档参考
    #
    ...
    #
    # 第四层（可选）：进程级隔离（_compile_code_in_subprocess）
    #   - 在独立子进程中执行技能代码，主进程不直接暴露给未受信任代码
    #   - 资源使用通过 subprocess timeout 限制
    #   - 即使前三层全部被绕过，攻击者也只能影响子进程
    #
    # 备选方案：当 RestrictedPython 不可用时，使用白名单内置函数 +
    # 代码静态审计作为备选防护。两者组合已能阻止所有已知沙箱逃逸路径。
    # =========================================================================

    # -------------------------------------------------------------------------
    # 严格白名单内置函数（Strict Whitelist of Safe Built-in Functions）
    # -------------------------------------------------------------------------
    # 安全准入标准（必须全部满足才能进入白名单）：
    #   ✗ 禁止任何 I/O 操作（文件 / 网络 / 进程 / 标准输入输出）
    #   ✗ 禁止任何反射操作（类型检查、属性遍历、类层次遍历）
    #   ✗ 禁止任何代码执行（import、eval、exec、compile）
    #   ✗ 禁止任何模块导入（直接或间接）
    #   ✗ 禁止任何全局状态修改（无副作用、引用透明）
    #
    # 允许函数清单（21 项）的安全考量：
    #   - True/False/None：不可变常量，无副作用
    #   - bool/float/int/str：纯类型构造函数，不访问任何外部资源
    #   - abs/divmod/max/min/pow/round/sum：纯数学函数，结果仅依赖入参
    #   - all/any/enumerate/len/list/range/sorted：纯序列/迭代操作，
    #     不修改入参、不进行 I/O、不访问反射接口
    #   - list 构造器虽然可以接收任意可迭代对象，但其本身不读取文件或网络；
    #     攻击者无法借助 list() 绕过沙箱（数据来源已被 _SAFE_BUILTINS 限制）
    # -------------------------------------------------------------------------
    _SAFE_BUILTINS = {
        # === 基础常量 (Constants) - 不可变、无副作用 ===
        "True": True,        # 布尔真值常量
        "False": False,      # 布尔假值常量
        "None": None,        # 空值常量

        # === 类型转换 (Type Conversions) - 纯函数、不访问外部资源 ===
        "bool": bool,        # 布尔类型转换，不触发 I/O
        "float": float,      # 浮点数类型转换
        "int": int,          # 整数类型转换
        "str": str,          # 字符串类型转换

        # === 纯数学运算 (Pure Math) - 无副作用、结果仅依赖入参 ===
        "abs": abs,          # 绝对值
        "divmod": divmod,    # 同时返回商和余数
        "max": max,          # 取最大值
        "min": min,          # 取最小值
        "pow": pow,          # 幂运算
        "round": round,      # 四舍五入
        ...
        # === 序列操作 (Sequence Operations) - 只读、不修改入参 ===
        "all": all,          # 所有元素为真则返回 True
        "any": any,          # 任一元素为真则返回 True
        "enumerate": enumerate,  # 返回 (index, item) 迭代器
        "len": len,          # 返回对象长度
        "list": list,        # 从可迭代对象构造列表（不触发 I/O）
        "range": range,      # 整数等差数列迭代器
        "sorted": sorted,    # 返回排序后的新列表（不修改入参）
    }

    # -------------------------------------------------------------------------
    # 明确禁止的内置函数清单 (Explicitly Forbidden Built-ins)
    # -------------------------------------------------------------------------
    # 此清单为不可变集合（frozenset），用作：
    #   1. 安全审计与代码审查时的参考依据
    #   2. 防止重构时意外将这些危险函数添加到 _SAFE_BUILTINS
    #   3. 文档化沙箱的安全边界，让审计人员明确"哪些函数绝不能出现"
    # -------------------------------------------------------------------------
    ...
    # 进程级隔离默认开关（可通过环境变量 SKILL_USE_SUBPROCESS=1 开启）
    # 启用后 _compile_code 将在独立子进程中执行未受信任代码，
    # 提供更强的进程级安全边界（参见 _SubprocessSkillExecutor）
    ...
    def _compile_code(self, code: str, skill_id: str) -> Optional[Callable]:
        ...
        安全策略（按优先级叠加，纵深防御）：
            第一层：代码静态审计（_audit_code_security）
                - 任何代码路径执行前必须先通过审计
                - 检测已知沙箱逃逸模式（__import__、__subclasses__、getattr 等）
                - 检测失败直接抛出 SecurityError，永不进入执行阶段

            第二层：RestrictedPython AST 级沙箱（若可用）
                - 使用 compile_restricted() 在 AST 层面阻止属性链访问
                - 工业级安全方案，可信度高于白名单内置方案
                - 依赖第三方库；不可用时回退到第三层

            第三层：白名单内置函数（_SAFE_BUILTINS，21 项严格白名单）
                - 备选方案；将 _SAFE_BUILTINS 作为 __builtins__ 注入
                - 沙箱内不存在的名称（包括 __import__/open/exec/eval/
                  getattr/type/dir 等）一律不可见

            第四层：进程级隔离（_SubprocessSkillExecutor，可选）
                - 通过环境变量 SKILL_USE_SUBPROCESS=1 启用
                - 在独立子进程中执行未受信任代码
                - 即使前三层全部被绕过，主进程仍不受影响

        返回技能中定义的第一个可调用入口点（execute / run / main / handler）。
        若代码有语法错误、审计不通过或沙箱执行异常，返回 None。
        """
        # --- 第一层：代码静态审计（所有路径都执行，绝不绕过）---
        self._audit_code_security(code, skill_id)

        # --- 第四层（可选）：进程级隔离 ---
        # 通过环境变量开启。启用时主进程不直接执行未受信任代码，
        # 而是返回 _SubprocessSkillExecutor 代理对象。
        ...
        try:
            ...
            # 沙箱内执行用户代码，可能抛出的异常族：
            # - SyntaxError：编译时语法错误（理论应在上游捕获）
            # - NameError：访问未列入白名单的 builtin
            # - AttributeError/TypeError/ValueError：用户代码逻辑错误
            # - RuntimeError：来自 RestrictedPython 的安全拦截
            # - OSError：受限 IO 操作（实际不应发生）
            logger.error(
    ...
    def _compile_code_in_process(self, code: str, skill_id: str) -> Optional[Callable]:
        ...
        使用经安全审计的 _SAFE_BUILTINS（21 项严格白名单）作为 __builtins__，
        阻止访问所有危险的内置函数。在执行前，代码已通过
        _audit_code_security() 的静态安全审计（第一层防护）。

        该方法仅作为 RestrictedPython 不可用时的备选方案。生产环境
        建议安装 RestrictedPython 库以获得 AST 级沙箱保护。
        """
        try:
        ...
        # __builtins__ 被替换为严格白名单，沙箱内任何对未列入白名单的
        # 内置函数（__import__/open/exec/eval/getattr/type/dir 等）的
        # 访问都会抛出 NameError，从语言层面阻止逃逸。
        namespace: Dict[str, Any] = {"__builtins__": self._SAFE_BUILTINS}
        try:
            exec(compiled, namespace)
        except (SyntaxError, NameError, AttributeError, TypeError,
                ValueError, RuntimeError, OSError) as e:
            # 主进程白名单沙箱中执行用户代码，异常族同 _compile_code。
            # 任一异常均表示技能代码不可用，应记录详细堆栈以便排查。
            logger.error(
                "Skill '%s' execution error: %s", skill_id, e, exc_info=True,
            )
            return None
    ...
    def _extract_callable(
        ...
        if "SkillExecutor" in namespace:
            ...
            try:
                ...
                # SkillExecutor 类的 __init__ 可能失败（参数错误、依赖缺失等）。
                # 失败时回退到查找其他可调用入口点。
                logger.debug(
        ...
        **kwargs,
        ...
        for skill in skills:
            try:
                ...
                # skill.execute 是用户代码入口，可能抛出任意业务异常。
                # 单个技能失败不应阻塞其他技能执行，逐条记录错误状态。
...
# =========================================================================
# 进程级隔离：_SubprocessSkillExecutor
# =========================================================================
# 第四层安全防护（可选）：在独立子进程中执行未受信任的技能代码。
#
# 设计动机：
#   纵深防御的前三层（白名单 / RestrictedPython / 静态审计）均在主进程内
#   运行。即使三层全部生效，未受信任代码依然在主进程地址空间内执行——
#   一旦未来发现新的沙箱逃逸漏洞，主进程的内存、文件描述符、网络连接等
#   都可能被攻击者利用。
#
#   进程级隔离通过以下方式提供更强的安全边界：
#     1. 独立子进程的内存空间与主进程隔离，攻击者无法直接读写主进程数据
#     2. 操作系统级别的资源限制（CPU 时间、内存）独立生效
#     3. 子进程崩溃或被攻击不会影响主进程的可用性
#     4. 通过 subprocess timeout 强制限制最长执行时间
#
# 进程间通信 (IPC) 设计：
#     - 输入：JSON 编码后通过 stdin 写入子进程
#       {"code": "<base64编码的技能代码>", "skill_id": "...", "args": {...}}
#     - 输出：JSON 编码后通过 stdout 读取
#       成功：{"status": "ok", "result": <JSON可序列化的返回值>}
#       失败：{"status": "error", "error": "...", "type": "ExceptionClass"}
#
# 限制与权衡：
#     - 启动子进程开销约 50-200ms，不适合高频调用场景
#     - 参数和返回值必须可 JSON 序列化（数字、字符串、列表、字典、布尔、None）
#     - 复杂 Python 对象（如自定义类、字节流）需要调用方自行序列化
#     - 在 Windows 上以 subprocess + Python 解释器方式实现，无需 multiprocessing
# =========================================================================
class _SubprocessSkillExecutor:
    """子进程隔离的技能执行代理。

    当被调用时，会在独立的 Python 子进程中执行技能代码，
    并通过标准输入/输出 + JSON 进行进程间通信。子进程使用与
    SkillLoader._SAFE_BUILTINS 完全一致的白名单内置函数。

    安全特性：
        - 子进程独立内存空间，主进程不受影响
        - 通过 subprocess timeout 限制执行时间（默认 10 秒）
        - 子进程结束即自动释放所有资源
        - 进程退出码被检查，非零退出视为失败

    使用方式：
        通常不需要直接构造此类的实例。SkillLoader._compile_code 在检测到
        环境变量 SKILL_USE_SUBPROCESS=1 时会自动返回该代理对象。
    """

    # Worker 脚本模板：在子进程中运行的最小化执行器
    # 该脚本使用与主进程 _SAFE_BUILTINS 完全相同的白名单，确保安全策略一致
    # 注意：这是一个 Python 源码字符串模板，不是 .format() 模板，
    # 因此内部使用单大括号 {} 即可，运行时字符串拼接用 + 或 str()。
    _WORKER_SCRIPT = '''\
# -*- coding: utf-8 -*-
"""子进程 worker 脚本 - 在隔离进程中执行技能代码。

由 _SubprocessSkillExecutor 自动写入临时目录并通过 sys.executable 调用。
该脚本本身是受信的（项目自有代码），仅负责在严格白名单下执行未受信任的技能代码。
"""
import sys
import json
import base64

# === 沙箱白名单（必须与 SkillLoader._SAFE_BUILTINS 严格一致）===
# 任何对未列入白名单的内置函数（__import__/open/exec/eval/getattr 等）
# 的访问都会抛出 NameError，从语言层面阻止逃逸。
SAFE_BUILTINS = {
...
'''
    ...
    def __init__(self, code: str, skill_id: str, timeout: float = 10.0):
        ...
        参数：
            code: 技能代码字符串（明文，仅在子进程内执行）
            skill_id: 技能标识符（用于日志和子进程内 __name__）
            timeout: 子进程最大执行时间（秒），超时将被强制终止
        """
        self.code = code
        self.skill_id = skill_id
        self.timeout = timeout
        # Worker 脚本延迟到首次调用时写入，避免无谓的文件 I/O
        self._worker_path: Optional[str] = None

    def _ensure_worker(self) -> str:
        """将 worker 脚本写入临时文件，返回脚本路径。"""
    ...
    def __call__(self, *args, **kwargs):
        ...
        与 in-process 执行器接口一致：接受任意关键字参数，将其作为
        技能入口点的命名参数调用。

        异常：
            TimeoutError: 子进程执行超过 timeout 秒
            RuntimeError: 子进程异常退出、输出无法解析或返回错误状态
        """
        import base64
        import subprocess
...
class _LoaderHolder:
    ...
    def init(self, skills_base_dir: Optional[str] = None) -> SkillLoader:
        """强制重新创建实例（用于启动时指定 skills_base_dir 的场景）。"""
        with self._lock:
            if self._instance is not None:
...
def get_skill_loader() -> SkillLoader:
    ...
    Returns:
        :class:`SkillLoader` 实例（应用生命周期内同一实例）。

    Note:
        同时也是 FastAPI 依赖工厂，可直接用于 ``Depends(get_skill_loader)``。
        实现是线程安全的，行为与重构前完全一致。
    """
    return _holder.get()
