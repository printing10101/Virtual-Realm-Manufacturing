...
class WorkflowSpec:
    ...
    def validate(self) -> list[str]:
        ...
        # 4. 节点 inputs 引用合法性（形如 ${node_id.output_name}）
        for node in self.nodes:
            for input_name, ref in node.inputs.items():
                ref_err = _validate_artifact_ref(ref, node_id_set, node.node_id, input_name)
                if ref_err:
                    errors.append(ref_err)

        # 5. 工作流 outputs 引用合法性
        for out_name, ref in self.outputs.items():
            ref_err = _validate_artifact_ref(ref, node_id_set, None, out_name)
            if ref_err:
                errors.append(ref_err)
...
def _validate_artifact_ref(
    ...
    """校验 ${node_id.output_name} 形式的 artifact 引用合法性."""
    if not ref.startswith("${") or not ref.endswith("}"):
        return f"artifact 引用格式错误（应为 ${{node_id.output_name}}）: {field_name}={ref}"
    inner = ref[2:-1]
    if "." not in inner:
        return f"artifact 引用缺少 . 分隔符: {field_name}={ref}"
