...
class BudgetEnforcer:
    ...
    def __init__(self, db_path: Optional[str] = None):
        if db_path is None:
            db_path = str(get_output_dir("data") / "budget_enforcer.db")
        ...
        self._pool = self._manager.get_pool("budget_enforcer")
...
class _BudgetEnforcerHolder:
    """Thread-safe lazy holder for the :class:`BudgetEnforcer` singleton."""
...
class _CostOptimizerHolder:
    """Thread-safe lazy holder for the :class:`CostOptimizer` singleton."""
...
def get_budget_enforcer() -> BudgetEnforcer:
    ...
    Note:
        同时也是 FastAPI 依赖工厂，可直接用于 ``Depends(get_budget_enforcer)``。
        实现是线程安全的，行为与重构前完全一致。
    """
    return _budget_holder.get()


def init_budget_enforcer(db_path: Optional[str] = None) -> BudgetEnforcer:
    """初始化预算执行器，行为与重构前完全一致。"""
    return _budget_holder.init(db_path)
...
def get_cost_optimizer() -> CostOptimizer:
    ...
    Note:
        同时也是 FastAPI 依赖工厂，可直接用于 ``Depends(get_cost_optimizer)``。
        实现是线程安全的，行为与重构前完全一致。
    """
    return _optimizer_holder.get()


def init_cost_optimizer() -> CostOptimizer:
    """初始化成本优化器，行为与重构前完全一致。"""
    return _optimizer_holder.init()
