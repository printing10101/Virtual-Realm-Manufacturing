...
def get(path):
    req = urllib.request.Request(
        f"http://localhost:8000{path}", headers={"Authorization": f"Bearer {TOKEN}"}
    )
...
for i in range(10):
    try:
        ...
        results.append(resp)
        print(
            f"    第{i + 1}次: 成功 - 推理耗时: {resp.get('data', {}).get('inference_time', 'N/A')}ms"
        )
...
try:
    ...
    print(
        f"    统计: {json.dumps(resp.get('data', resp), indent=2, ensure_ascii=False)}"
    )
except Exception as e:
    print(f"    查询失败: {e}")
