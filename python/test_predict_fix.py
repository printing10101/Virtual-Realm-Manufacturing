...
code, resp = post("/api/v1/lnn/predict", {
    "model_name": "cutting_force",
    "input_data": [120.5, 85.3, 65.1, 12000.0, 0.15]
})
print(f"  Status: {code}")
print(f"  Response: {json.dumps(resp, indent=2, ensure_ascii=False) if isinstance(resp, dict) else resp[:300]}")
if code == 200:
...
code, resp = post("/api/v1/lnn/predict", {
    "model_name": "surface_roughness",
    "input_data": [120.5, 85.3, 65.1, 12000.0]
})
print(f"  Status: {code}")
print(f"  Response: {json.dumps(resp, indent=2, ensure_ascii=False) if isinstance(resp, dict) else resp[:300]}")
if code == 200:
